...
sparsifying_functions = {
    "louvain":  lambda adj_matrix: local_top_percent_union( relu(zero_diag(adj_matrix)), top_percent),
    "leiden":lambda adj_matrix: local_top_percent_union( zero_diag(adj_matrix), top_percent),
}


def graph_pipeline(adj_matrix, searched_formulas, output_dir, name):
    ...
    k_core_nodes_df, k_core_stats_df = analyze_k_core(G)
