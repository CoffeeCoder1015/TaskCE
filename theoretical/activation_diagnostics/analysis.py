...
def run(
    ...
    *,
    ...
    raw_result = raw_activation_analysis(
        base_states,
        finetuned_states,
        output_dir=output_directory,
    )
    alpha_result = run_alpha_sweep(
        finetuned_states,
        task_name=task_name,
        alpha=alpha,
        min_acts=min_acts,
    ...
    thresholds = torch.quantile(finetuned_states, 1 - alpha, dim=0)
    binary_result = binary_activation_analysis(
        finetuned_states > thresholds,
        min_acts=min_acts,
        output_dir=output_directory,
    )
    return {
        "raw": raw_result,
        "alpha_sweep": alpha_result,
...
def compare_raw_activations(base_states, finetuned_states):
    """Compare paired raw activation matrices."""
    result = raw_activation_analysis(base_states, finetuned_states)
    return {
